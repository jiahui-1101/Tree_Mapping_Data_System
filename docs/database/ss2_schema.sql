...
CREATE TABLE IF NOT EXISTS ss2_field_tasks (
  ...
  status ENUM('pending', 'in-progress', 'completed', 'escalated', 'skipped', 'false-positive', 'anomaly-found') NOT NULL DEFAULT 'pending',
  notes TEXT,
...
CREATE TABLE IF NOT EXISTS ss2_field_reports (
  ...
  treatment TEXT,
  notes TEXT,
  height_measurement DECIMAL(5,2),
  gps_label VARCHAR(180),
  timestamp_label VARCHAR(80),
  sync_status VARCHAR(40) NOT NULL DEFAULT 'synced',
  review_status ENUM('Pending Review', 'Approved', 'Reassigned') NOT NULL DEFAULT 'Pending Review',
  analysis JSON,
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
