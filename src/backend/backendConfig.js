...
export function getBackendConfig(env = process.env) {
  ...
  return {
    ...
    ss4StorePath: env.SS4_STORE_PATH || path.join(process.cwd(), ".runtime", "ss4-store.json"),
    itSupportStorePath: env.IT_SUPPORT_STORE_PATH || path.join(process.cwd(), ".runtime", "it-support-store.json"),
    ss4Store: (env.SS4_STORE || "json").toLowerCase(),
    itSupportStore: (env.IT_SUPPORT_STORE || "json").toLowerCase(),
    ss4AiProvider: (env.SS4_AI_PROVIDER || env.SS3_AI_PROVIDER || "mock").toLowerCase(),
